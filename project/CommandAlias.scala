...
object CommandAlias {
  ...
  val allDockerAlias: Seq[Def.Setting[_]] =
    dockerCommands("travisci", kafkaProducerKey, kafkaConsumerKey) ++
      dockerCommands(kafkaProducerKey) ++ 
      dockerCommands("kpu", kafkaProducerKey) ++
      dockerCommands(kafkaConsumerKey) ++
      dockerCommands("kcu", kafkaConsumerKey)
